...
public class CollectionDemoSet {
    public static void main(String[] args) {
        Scanner userInput = new Scanner(System.in);  //Siempre es necesario crear un scanner en java para que lea
        Set<String> frutas = new HashSet<>(); // Aqui le decimos el tipo de set que haremos. Nuestro set sera hashSet
        System.out.println("Bienvenido al set de frutas. Usted ingresa las frutas que compro en esta caja y yo le ayudo a buscarlas despues.");
        System.out.println("Sepa que no me interesan las mayusculas y minusculas. Todo es lo mismo para mi!");
        String fruit = null;  // Aqui no hay frutas aun!
        String choice = "1"; // Comienzo en 1 para invocar while.



        while (choice.equalsIgnoreCase("1")) {
            System.out.println("Ingrese una fruta a la caja: ");
            fruit = userInput.nextLine();
            frutas.add(fruit.toLowerCase());
            System.out.println("Presione 1 para agregar mas frutas, cualquier caracter para terminar");
            choice = userInput.nextLine();
        }

        System.out.println("Revisemos si la fruta que quieres esta en la caja.");
        System.out.println("Escriba una fruta a buscar:");
        String frutaBuscada = userInput.nextLine();
        if (frutas.contains(frutaBuscada.toLowerCase())) {
            System.out.println("Si! \"" + frutaBuscada.toLowerCase() + "\" está en la caja.");
        } else {
            System.out.println("No hay \""+ frutaBuscada.toLowerCase() +"\" en la caja :(");
        }

        // Como recorrer todas las frutas de la caja, y ver que los duplicados fueron vilmente ignorados:
        System.out.println("Aqui van todas las frutas de nuestro catalogo:");
        for (String fruta : frutas) {
            System.out.println(fruta);
        }
    }
